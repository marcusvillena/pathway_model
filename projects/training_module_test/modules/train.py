...
class MultiTrainingModule():
    def __init__(self, model_class, model_kwargs:dict, training_class, training_kwargs:dict, num_trials:int, comment:str = None, autorun:bool=True):
        # inst vars
    ...
    def run(self):
        # get dir
        self.folder = self._get_dir(self.comment)
        ...
        for trial in tqdm(range(self.num_trials)):
            ...
            training_module = self.training_class(model=model, **self.training_kwargs)
    ...
    def _get_summary(self):
        ...
        summary.to_csv(self.folder / 'summary.csv')

        return summary

    def _plot(self, metric:str, title:bool=True, save_fig:bool=True):
        # copy data
        train = self.train_results.copy()
        val = self.val_results.copy()
        ...
        # save plot
        if save_fig == True:
            fig.savefig(self.folder / f'{self.comment}_{metric}.svg')

        return fig
    
    def _multi_plot(self):
        # get list of metrics
        metrics = self.summary.columns

        # init plot dict
        figs = {}

        for metric in metrics:
            figs[metric] = self._plot(metric)
            plt.close()
