...
from .layersT import SetPooling, Sequential
from .utils import attn_dims, cloneable, input_to_dict, reshape, tsoftmax
...
class NodePooling(nn.Module):
    ...
    def forward(self, input:Union[Data, Tensor, dict], need_weights:bool=False, temperature:Optional[float]=1):
        # extract h_node
        data = input_to_dict(input)
        ...
        pool_out = self.set_pooling(h_node, concat=concat, return_dict=need_weights, temperature=temperature)
...
class Encoder(nn.Module):
    ...
    def forward(self, x:Union[Data, Tensor, dict], need_weights:bool=False, temperature:Optional[float]=1):
        # nb pass
        if self.nb is not None:
            x = self.nb(x, need_weights)

        # encoder pass
        x = self.encoder(x, need_weights, temperature=temperature)

        # pooling pass
        if self.pooling is not None:
            x = self.pooling(x, need_weights, temperature=temperature)
...
class Latent(nn.Module):
    ...
    def _pooling(self, pooling_class, x, need_weights:bool=False, temperature:Optional[float]=1):
        if need_weights: # pooling outputs dict
            out = pooling_class(x, concat=False, return_dict=need_weights, temperature=temperature)
            z = out.get('x')
            attn = out.get('attn')

        else: # pooling outputs z (Tensor)
            z = pooling_class(x, concat=False, return_dict=need_weights, temperature=temperature)
            attn = None
    ...
    def forward(self, input:Union[Data, Tensor, dict], need_weights:bool=False, temperature:Optional[float]=1):
        ...
        # pool
        if self.method == 'node':
            z_node, attn_node = self._pooling(self.node_pool, x, need_weights, temperature=temperature)
            z_set, attn_set = None, None
        elif self.method == 'set':
            z_node, attn_node = None, None
            z_set, attn_set = self._pooling(self.set_pool, x, need_weights, temperature=temperature)
        else: # self.method == 'twin'%%!
            x_node, x_set = x.split(self.split_dim, dim=1) #split, pool sep
            z_node, attn_node = self._pooling(self.node_pool, x_node, need_weights, temperature=temperature)
            z_set, attn_set = self._pooling(self.set_pool, x_set, need_weights, temperature=temperature)
        ...
        if self.fwd == 'node':
            ...
            if self.fwd == 'twin_pool':
                x, attn_twin = self._pooling(self.twin_pool, x, need_weights, temperature=temperature)
            else:
                x = x.mean(dim=1)
                attn_twin = None
...
class Autoencoder(nn.Module):
    ...
    def forward(self, input:Union[Data, Tensor, dict], need_weights:bool=False, temperature:Optional[float]=1):
        x = self.encoder(input, need_weights=need_weights, temperature=temperature)
        x = self.latent(x, need_weights=need_weights, temperature=temperature)
    ...
    def get_weights(self, input:Union[Data, Tensor, dict], temperature:Optional[float]=1):
        x = self.forward(input, need_weights=True, temperature=temperature)
        return x
